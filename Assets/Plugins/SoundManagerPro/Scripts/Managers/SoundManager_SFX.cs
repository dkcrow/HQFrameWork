...
public partial class SoundManager : antilunchbox.Singleton<SoundManager> {
    ...
    public static AudioSource PlaySFX3D(GameObject gO, string clipName)
    {
        if (Instance.offTheSFX || Instance.isPaused)
            return null;
        ...
        if (audioS == null)
        {
            audioS = gO.AddComponent<AudioSource>();
            audioS.playOnAwake = false;
            audioS.spatialBlend = 1.0f;
            audioS.dopplerLevel = 0.0f;
            audioS.minDistance = 5.0f;
            audioS.maxDistance = 50.0f;
        }
        ...
        return PlaySFX(gO.GetComponent<AudioSource>(), SoundManager.Load(clipName));
    }
